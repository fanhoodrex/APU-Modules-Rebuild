...
summary(Hourly.weather.data)

# Get the sum for all the missing value for each column
#Using the sapply function
sapply(Hourly.weather.data, function(x) sum(is.na(x)))

# replacing the missing value by the mean for pressure column

Hourly.weather.data$pressure = ifelse(is.na(Hourly.weather.data$pressure),
                                      ave(Hourly.weather.data$pressure,
                                          FUN = function(x) 
                                            mean(x, na.rm = 'TRUE')),
                                      Hourly.weather.data$pressure)

# Replacing the column by Zero for the all values in dewp Columns
Hourly.weather.data[["dewp"]][is.na(Hourly.weather.data[["dewp"]])] <- 0
head(Hourly.weather.data$dewp)

# checking the outlier for pressure
ggplot(Hourly.weather.data, mapping = aes(x = 1, y = pressure, fill = pressure)) + 
...
Hourly.weather.data$pressure=outlier_norm(Hourly.weather.data$day)
ggplot(Hourly.weather.data, mapping = aes(x = 1, y = pressure, fill = pressure)) + 
...
ggplot(Hourly.weather.data, aes(wind_speed, day, color = "red")) +
...
theme_set(theme_classic())+ filter(Hourly.weather.data, month = 3, day = 10)
g <- ggplot(Hourly.weather.data, aes(wind_gust)) + scale_fill_brewer(palette = "Spectral")
...
g <- ggplot(Hourly.weather.data, aes(x=day, wind_dir,grouping(month)))
...
Hourly.weather.data$temp_status <-
  ifelse(Hourly.weather.data$temp >= 0 & Hourly.weather.data$temp <= 20, 'low',
         ifelse(Hourly.weather.data$temp >=20 & Hourly.weather.data$temp <=30, 'medium',
                ifelse(Hourly.weather.data$temp >=30, 'High', ' High')))

# Barchart of the temp_status
ggplot(data = Hourly.weather.data,aes(x= temp_status,fill = temp_status))+ 
  geom_bar()+ 
  labs(title = "Barchart of the temp_status")

# count plot between day and month
ggplot(Hourly.weather.data, aes(x = day, y = month)) + 
  geom_count() +
  labs(title="The co-variation between day and month",
       x="day", y="month")

#Line between hour and wind_speed
Mitty <- ggplot(Hourly.weather.data, aes(x = wind_speed, y = hour ))
...
ggplot(Hourly.weather.data, aes(x = hour, y = temp_status)) +
...
ggplot(Hourly.weather.data, aes(x=hour, y=visib)) + 
